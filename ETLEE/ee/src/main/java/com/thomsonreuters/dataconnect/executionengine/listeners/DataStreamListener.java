package com.thomsonreuters.dataconnect.executionengine.listeners;
...
import com.thomsonreuters.dataconnect.executionengine.exceptionhandler.ListenerException;
...
import com.thomsonreuters.dataconnect.executionengine.services.DataSyncExecutionEngine;
import com.thomsonreuters.dataconnect.executionengine.utils.ExecutionContextUtil;
...
import static com.thomsonreuters.dataconnect.executionengine.constant.Constants.ACCEPTED;
...
public class DataStreamListener {
    ...
    @RabbitListener(id = "dataStreamListener", queues = "${spring.rabbitmq.connections.data-sync.source.source.queue.name}", containerFactory = "rabbitDataStreamListenerContainerFactory")
    public void listenAndConsume(DatasyncMessage dataSyncMessage, Message message, Channel channel) throws IOException, InterruptedException {
        try {
            log.debug("MsgRecvdTime: {}, DataStreamListener_Thread: {}, ConsumerTag: {}, JobExecutionId: {}",
                    LocalDateTime.now(),
            ...
            log.info("DataStreamListener Target region receiving queue: {}", execId);
            ...
            // dataSyncExecutionEngine.updateExecutionStatus(ctx,ACCEPTED);
