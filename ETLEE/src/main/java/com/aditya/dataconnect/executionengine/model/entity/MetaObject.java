...
public class MetaObject extends BaseEntity {
    ...
    @Column(name = "db_table", unique = true)
    @JsonProperty("db_table")
    private String dbTable;

    @Column(name = "schema", unique = true)
    @JsonProperty("schema")
    ...
    @JsonIgnore
    @OneToMany(fetch = FetchType.EAGER, mappedBy = "metaObject", cascade = CascadeType.ALL, orphanRemoval = true)
    private Set<MetaObjectAttribute> attributes = new HashSet<>();


    @JsonIgnore
    @OneToMany(fetch = FetchType.EAGER, mappedBy = "parentObject", cascade = CascadeType.ALL, orphanRemoval = true)
