...
public class SQLBuilder {
    ...
    public static String selectSpecificColumnsFromTable(MetaObjectDTO parentMetaObject, String keyColName, String specificColumn, int objectIdCount) {
        StringBuilder select = new StringBuilder("SELECT ");
        ...
        if (objectIdCount > 1) {
            // Use IN clause for multiple object IDs
            select.append(" WHERE ").append(keyColName).append(" IN (");
            for (int i = 0; i < objectIdCount; i++) {
                select.append("?");
                if (i < objectIdCount - 1) {
                    select.append(", ");
                }
            }
            select.append(")");
        } else {
            // Use single = for one object ID
    ...
    public static String selectSpecificColumnsFromTable(MetaObjectDTO parentMetaObject, String keyColName, String specificColumn) {
        // Backward compatibility method - defaults to single object ID
        return selectSpecificColumnsFromTable(parentMetaObject, keyColName, specificColumn, 1);
    }
